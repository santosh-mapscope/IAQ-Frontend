...
const ExternalDocumentModal = ({ reportId, closeModal, showExternalModal }) => {
  ...
  const handleUpload = async () => {
    if (reportId && uploadedFiles.length > 0 && !isUploading) {
      const formData = new FormData();  
       ...
       // Append each file to the formData using a loop
      ...
      Swal.fire({
        ...
        text: "Do you want to upload external document(s) for this building?",
        icon: "warning",
        ...
        if (result.isConfirmed) {
          ...
          axiosInstance
            .post("pdf/uploadExternalDocuments", formData, { headers: headersForJwt })
            .then((res) => {
